...
func getUserRouter(r *mux.Router, userHandler *handlers.UserHandler) {
	...
	protectedAdminRouter.HandleFunc("/all-users", userHandler.GetAllUsers).Methods(http.MethodGet)
	protectedAdminRouter.HandleFunc("/active-users", userHandler.GetActiveUsers).Methods(http.MethodGet)
	protectedAdminRouter.HandleFunc("/admin-users", userHandler.GetAdminUsers).Methods(http.MethodGet)
	protectedAdminRouter.HandleFunc("/super-admin-users", userHandler.GetSuperAdminUsers).Methods(http.MethodGet)
	protectedAdminRouter.HandleFunc("/deleted-users", userHandler.GetDeletedUsers).Methods(http.MethodGet)
	protectedAdminRouter.HandleFunc("/inactive-users", userHandler.GetInactiveUsers).Methods(http.MethodGet)
	protectedAdminRouter.HandleFunc("/suspended-users", userHandler.GetSuspendedUsers).Methods(http.MethodGet)
}